...
XlsxStreamReaderWorkSheet.prototype._handleWorkSheetNode = function (nodeData) {
  ...
  if (self.inRows === true) {
    ...
    if (workingCell.name === 'c') {
      ...
      switch (workingCell.attributes.t) {
        ...
        default: // eslint-disable-line no-fallthrough
          if (self.options.formatting && self.workBook.xfs[workingCell.attributes.s]) {
            if (self.workBook.hasFormatCodes) {
              var formatId = self.workBook.xfs[workingCell.attributes.s].attributes.numFmtId
    ...
    if (workingCell.name === 'v') {
      self.workingRow.values[self.workingRow.values.length - 1] = workingPart || ''
    }
  } else {
